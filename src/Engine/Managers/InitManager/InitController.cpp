...
void InitController::RegisterComponent(const std::string& componentTypeName, const std::string& componentName, const std::vector<std::string>& dependencies) {
    if (m_componentDependencies.count(componentName)) {
		...
		OutputDebugStringA("[InitController] Advertencia: Componente ya registrado. Sus dependencias serán sobrescritas.\n");
    }	
    m_componentDependencies[componentName] = { dependencies, componentTypeName };
    OutputDebugStringA(("[InitController] Componente registrado: " + componentName + " - OK\n").c_str());
}
    ...
    for (const auto& pair : m_componentDependencies) {
        ...
        if (visited.find(pair.first) == visited.end()) {
            ...
            if (!DFS(componentTypeName, pair.first, visited, visiting)) {
                throw std::runtime_error("[InitController] ERROR FATAL: Se detectó un ciclo de dependencia durante el cálculo del orden de inicialización.");
            }
        }
    }
    ...
    if (it != m_componentDependencies.end()) {
        for (const std::string& dep : it->second.first) {
            ...
            if (m_componentDependencies.find(dep) == m_componentDependencies.end()) {
                std::cerr << "[InitController] Advertencia: El componente '" << componentName << "' depende de '" << dep << "', pero '" << dep << "' no se ha registrado como un componente. Se ignorará esta dependencia." << std::endl;
                continue;
            }
            ...
            if (visiting.find(dep) != visiting.end()) {
                std::cerr << "[InitController] ERROR: Ciclo de dependencia detectado: '" << componentName << "' -> ... -> '" << dep << "'." << std::endl;
        ...
        std::cerr << "[InitController] ERROR Interno: Intento de procesar componente no registrado en DFS: '" << componentName << "'." << std::endl;
        return false;
    }
