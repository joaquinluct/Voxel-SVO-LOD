...
#define DEFINE_INIT_ASSET_LAMBDA(AssetClassName) \
    [](std::shared_ptr<IAsset> servicePtr) -> HRESULT { \
        ...
        public: \
            Asset ## AssetClassName ## Registrar() { \
                AssetLocator::RegisterAssetCreator( \
                    AssetNameString, \
                    /* Lambda de creación: Devuelve std::shared_ptr<AssetClassName> que se convierte implícitamente a std::shared_ptr<IAsset> */ \
