...
void RenderManager::RunLoop() {
    ...
    if (!m_sceneManager->WaitForFlag(SyncFlagIndex::HasScene, true, std::chrono::seconds(5))) {
...
std::shared_ptr<RenderPass> RenderManager::InitPass(std::string passName) {
    ...
    // 1. Actualizar la informaici�n del pase en el FrameState
...
// Tambi�n prepara los valores por defecto de los pases y los a�ade al mapa de pases
// para que est�n disponibles durante el render
// ----------------------------------------------------------
...
void RenderManager::Render() {
    ...
    // Bloquear el FrameStateService para evitar conflictos con el hilo de actualizaci�n.
    m_frameStateService->BeginRendering();
    //m_frameStateService->Lock(FRAME_STATE_RENDER);

    // Paso 2.1: Obtener la lista de comandos iniciales grabada por el hilo de actualizaci�n.
    //m_frameStateService->Lock(FRAME_STATE_RENDER);
    ...
    // Pre-bind terrain resources (vertex/index/instance CB) so deferred pass operations find them ready
    if (m_terrain) {
        m_terrain->BindForRender(m_context.Get());
    }
    ...
    // Desbloquear el FrameStateService para permitir que el hilo de actualizaci�n contin�e.
    //m_frameStateService->Unlock(FRAME_STATE_RENDER);
...
//		// Lo mismo con culquier iteraci�n que se haga a partir de ahora y sobretodo dentro del bucle de meshes
//		auto& passShared = passPair.second;
//		auto* pass = passShared.get();
//
//		// 0. Enviar configuraci�n del pase al FrameState
...
//				// 4.4 A�adir las operaciones del mesh al buffer de operaciones del pase
...
//
//		// 6. A�adir las operaciones al buffer de operadciones
...
// --------------------------------------------------------------------------
// M�todos de gesti�n del buffer de operaciones
// --------------------------------------------------------------------------
