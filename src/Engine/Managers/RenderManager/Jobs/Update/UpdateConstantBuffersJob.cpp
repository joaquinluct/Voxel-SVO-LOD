...
bool UpdateConstantBuffersJob::Execute(JobContext* context)
{
	...
	lightParams.fogStartDistance = 5000.0f; // Empieza a desvanecer la geometría
	lightParams.fogEndDistance = 7000.0f; // Totalmente gris, ocultando el fallo del LOD
	...
	lightParams.fogColor = XMFLOAT3(0.2f, 0.2f, 0.23f);
	lightParams.fogHeightFalloff = 0.02f; // Pequeño valor (ej. 0.05 a 0.5) para controlar la rapidez con la que se disipa la niebla hacia el cielo.
