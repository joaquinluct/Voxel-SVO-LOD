...
void UIPass::SetInitialOperations() {	
	AddInitialOperation(PipelineOperationType::Device_ResetRenderTargetView);
	...
	AddInitialOperation(PipelineOperationType::Device_EnabledBledingState);
}
...
std::map<std::string, std::shared_ptr<MeshAsset>> UIPass::GetMeshes(const SceneManager* SceneManager, FrameStateService* renderState) {
	return {};
	//return SceneManager->GetMeshesByRenderPass(RenderPassType::UIPass);
}

std::vector<PipelineOperationType> UIPass::BeginPass(const MeshAsset* mesh, FrameStateService* renderState)
{
...
std::vector<PipelineOperationType> UIPass::ExecPass(const MeshAsset* mesh, FrameStateService* renderState)
{
	ClearOperations();
...
//	std::map<std::string, std::shared_ptr<MatrixDefinitionBase::IMatrixParams>> mConstantsBuffers = m_initManager->GetConstantsBuffers();
//	PipelineMatrixBufferData matrixData = {};
//	//matrixData.params = mConstantsBuffers;
//	matrixData.params = renderState->MaterialState()->GetConstantBuffers();
...
std::vector<PipelineOperationType> UIPass::EndPass()
{
	return {};
}