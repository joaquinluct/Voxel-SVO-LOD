...
m_fieldOfView(XM_PIDIV4), m_aspectRatio(1.0f), m_nearPlane(0.1f), m_farPlane(3000.0f), m_cameraSpeed(CAMERA_SEEP), m_keyboardManager(nullptr) {
	m_keyboardManager = ServiceLocator::GetService<KeyboardManager>();
}
...
void Camera::Update(float deltaTime) {
    ...
    //if (m_keyboardManager) {
    //    // Activar/desactivar la gravedad con la tecla Tabulador
    //    if (m_keyboardManager->IsKeyPressed(VK_TAB)) {
    //        ToggleGravity();
    //    }
    ...
    //        // Simulación básica de colisión con el suelo (opcional)
    //        if (position.y < 2.0f) { // Ajusta este valor según la altura del suelo
    //            position.y = 2.0f;
    //            m_verticalVelocity = 0.0f; // Detener la caída
    //        }
    //    }

    //    // Salto con la barra espaciadora
    //    if (m_keyboardManager->IsKeyPressed(VK_SPACE) && position.y == 2.0f) { // Solo saltar si está en el suelo
    //        m_verticalVelocity = JUMP_SPEED;
    //        m_gravityEnabled = true; // Activar la gravedad al saltar
    //    }

    //    // Movimiento de la cmara (horizontal)
    ...
    //    XMVECTOR forward = XMVectorSet(0.0f, 0.0f, forwardMove, 0.0f);
    //    XMVECTOR strafe = XMVectorSet(strafeMove, 0.0f, 0.0f, 0.0f);
    ...
    //    Move(s.x * m_cameraSpeed * deltaTime, 0.0f, s.z * m_cameraSpeed * deltaTime); // Movimiento horizontal
...
Util::Triangle* Camera::GetTriangleLookingAt(const std::vector<Util::Triangle>& triangles) const {
    ...
    // Crear una instancia de la clase RayTracing
    ...
    for (auto& triangle : triangles) {
        float distance = rayTracer.Trace(cameraPosition, cameraDirection, 1000.0f, { triangle }); // Traza contra un solo triángulo
