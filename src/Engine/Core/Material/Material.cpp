...
Material::Material(std::wstring shaderName) : m_shaderName(shaderName), m_texture(nullptr), m_samplerState(nullptr), m_matrixBuffer(nullptr) {
	std::shared_ptr<ShaderManager> shaderManager = ManagerLocator::GetShaderManager();
...
HRESULT Material::Init(ID3D11Device* device, TextureType textureType) {
    HRESULT hr = S_OK;

	Release(); // Asegurarse de liberar recursos previos
...
void Material::SetShaderParameters(ID3D11DeviceContext* context, const DirectX::XMMATRIX& worldMatrix, const DirectX::XMMATRIX& viewMatrix, const DirectX::XMMATRIX& projectionMatrix, int slot) {
    ...
    context->VSSetConstantBuffers(slot, 1, &m_matrixBuffer); // Slot 0 (b0)
}
...
ID3D11ShaderResourceView* Material::LoadTextureFromFile(ID3D11Device* device, const std::wstring& filename) {
    ID3D11ShaderResourceView* texture = nullptr;
    HRESULT hr = DirectX::CreateWICTextureFromFile(device, filename.c_str(), nullptr, &texture);
    if (FAILED(hr)) {
