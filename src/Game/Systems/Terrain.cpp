...
#include "Terrain/ProceduralService.h"
...
HRESULT Terrain::InitServices()
{
	...
	m_terrainAsset = AssetLocator::GetTerrainAsset("ProceduralTerrain");
	m_terrainAsset->Init();
	return S_OK;
}
...
std::vector<std::shared_ptr<Chunk>> Terrain::GetChunks(std::shared_ptr<ICamera> camera) {
	if (!m_chunkService) {
		return {};
	}
	...
	std::vector<std::shared_ptr<Chunk>> visibleChunks = m_chunkService->GetFrustumChunks(frustumPlanes, cameraPosition);
	...
	std::vector<std::shared_ptr<Chunk>> finalVisibleChunks;
	...
	std::vector<std::shared_ptr<Chunk>> occluderChunks;
