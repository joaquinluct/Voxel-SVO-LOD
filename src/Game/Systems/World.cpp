...
HRESULT World::InitSystems() {
    ...
    bool hasBiome = WorldDefinition::HasBiome(m_gameFlags);
    this->flags.SetFlag(FLAG_WORLD_HAS_BIOME, hasBiome);
    if (hasBiome) {
        m_biome = ServiceLocator::GetService<Biome>();
        if (!m_biome) {
            return E_FAIL;
        }
        hr = m_biome->Init();
...
DirectX::XMFLOAT3 World::GetTerrainNormal(float posX, float posZ) {
    if (m_terrain) {
        return m_terrain->GetTerrainNormal(posX, posZ);
    }
}
